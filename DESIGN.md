...
- **D32 (2026-08-28) — One telephony contract, per-customer adapters; Telavox first.** There is no universal standard for noticing incoming calls on a PC: TAPI is the legacy Windows surface (PBX-vendor TSPs, fading), CSTA the PBX-side CTI protocol, and the practical reality is per-vendor cloud APIs. Customer setups will vary widely and that is planned for, not worked around: we define one standard internal **call contract** — events in (`incoming_call(number, timestamp)`, later optionally answered/ended) and actions out (`dial(number)`, defaulting to a tel:/provider link, API dial where offered) — and behind it a deliberately **specific solution per installation**, authored at onboarding exactly like D29 product adapters (bespoke integration work is AI-assisted development time, never runtime machinery). Adapters declare what they can deliver, and the panel degrades gracefully along that capability ladder: full screen-pop with pre-filled brief → numberless "call started?" prompt (audio-activity heuristic) → manual start. The wrapping — contract, capability declaration, number→customer resolution, panel behavior — is ours and identical everywhere; only the thin provider edge is bespoke. Decisions:
  - **Flow:** backend gains a small ringing endpoint; on signal it resolves number → customer via the CRM index, prepares the brief (D28), and pushes an incoming-call event over the SignalR hub to the rep's panel — pre-call card arrives pre-filled, one click to start as the rep answers. This closes the D30 loop for inbound, where reps are least prepared.
...
- **D33 (2026-08-28) — Document-source product data: extraction at development time, verification at the waist.** Test Power's catalog arrived as 48 Yokogawa brochures (PDF, English, text layer, model-and-suffix-code tables) — a *document* source, not a feed. D29 still holds: the pipeline consumes only canonical JSONL, and the adapter is authored at onboarding. What changes is the adapter's nature: for documents the model codes, specs and relations are **extracted by an LLM pass at development time** (D29's "AI at development time, never runtime machinery" — literally), and the D29 hard rule that IDs never pass through an LLM becomes **verification at the waist**: every extracted model code must appear verbatim in the source document or the row is dropped with a report line; relations whose target was not extracted anywhere are dropped likewise. Prices and availability never come from documents — they stay null until a structured source (Test Power's price list / ERP export) is merged by model code. Decisions:
  - **Granularity:** every orderable thing is a product — base instrument, option (`/G7`), module (`760901`), accessory (probes), software — with typed relations to its base (`option_of`, `module_of`, `accessory_of`, `software_for`); suffix codes that only select language or power cord stay as attributes. This is what lets the advisor surface an option or a probe as the complement — the cross-sell the copilot exists for.
  - **Extractor — subscription first, API by explicit approval.** The default path is Claude Code authoring the per-document extraction in-session on the developer subscription (D27's stated role; zero marginal cost) into the tool's cache files; the tool then verifies, merges and validates offline. The API path (Opus 5, high effort, one brochure per call, cached per content hash) exists behind an explicit `--allow-api` flag and is used only with a per-run go-ahead that states the cost. *Lesson 2026-09-05:* the first Yokogawa extraction ran on API credits (~$30–35 incl. a crash and truncation retries) without that go-ahead — recorded so it does not recur. The remaining brochure (DLM oscilloscopes, 143 rows) was then authored in-session at zero cost, giving the full Test Power catalog: 908 products, 131 instruments, 31 families, 0 unverifiable codes.
