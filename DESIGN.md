...
- **D32 (2026-08-28) — Telephony signal adapters; Telavox first.** There is no universal standard for noticing incoming calls on a PC: TAPI is the legacy Windows surface (PBX-vendor TSPs, fading), CSTA the PBX-side CTI protocol, and the practical reality is per-vendor cloud APIs — so call signals follow the same thin-adapter pattern as product feeds (D5/D29) and CRM briefs (D28): a per-installation **telephony adapter** that turns the provider's event surface into one internal signal, `incoming_call(number, timestamp)`. Outbound needs no adapter — click-to-call links (tel:/provider links) already work. Decisions:
