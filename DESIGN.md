...
- **D33 (2026-08-28) — Document-source product data: extraction at development time, verification at the waist.** Test Power's catalog arrived as 48 Yokogawa brochures (PDF, English, text layer, model-and-suffix-code tables) — a *document* source, not a feed. D29 still holds: the pipeline consumes only canonical JSONL, and the adapter is authored at onboarding. What changes is the adapter's nature: for documents the model codes, specs and relations are **extracted by an LLM pass at development time** (D29's "AI at development time, never runtime machinery" — literally), and the D29 hard rule that IDs never pass through an LLM becomes **verification at the waist**: every extracted model code must appear verbatim in the source document or the row is dropped with a report line; relations whose target was not extracted anywhere are dropped likewise. Prices and availability never come from documents — they stay null until a structured source (Test Power's price list / ERP export) is merged by model code. Decisions:
  ...
  - **Extractor:** Opus 5 at high effort, one brochure per call, strict schema, raw responses cached per document content hash so re-runs re-pay only for changed editions (D29's incremental principle). One-time cost for the pilot catalog ≈ $20–30.
